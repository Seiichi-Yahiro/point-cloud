// Copied from https://github.com/NavyaZaveri/lfu-cache
// Extended so that evict returns the evicted key value pair
...
impl<K: Hash + Eq, V> LFUCache<K, V> {
    ...
    fn evict(&mut self) -> (K, V) {
        ...
        let value = self.values.remove(&least_recently_used).unwrap().value;

        let key =
            Rc::try_unwrap(least_recently_used).unwrap_or_else(|_| panic!("Key was still shared"));
        (key, value)
    }
    ...
    pub fn set(&mut self, key: K, value: V) -> Option<(K, V)> {
        ...
        let evicted_pair = if self.len() >= self.capacity {
            Some(self.evict())
        } else {
            None
        };
